...
| **Audit controls (b)** — record and examine activity involving ePHI | ◐ | An on-device, append-only audit log of PHI-affecting actions (record opened/exported/deleted, app unlocked, encryption toggled, backup run), viewable and exportable, stored only on the Mac. |
