...
public enum ChunkedCipher {
    public static let magic = Data("ALTC".utf8)
    public static let version: UInt8 = 1
    public static let defaultChunkSize = 1 << 20 // 1 MiB

    private static var headerCount: Int { magic.count + 1 + 4 + 8 }

    public enum ChunkedError: LocalizedError, Equatable {
        case notAnEnvelope
    ...
    public static func isEnvelope(fileAt url: URL) -> Bool {
    ...
    public static func seal(fileAt src: URL, to dst: URL, using key: SymmetricKey, chunkSize: Int = defaultChunkSize) throws {
    ...
    public static func open(fileAt src: URL, to dst: URL, using key: SymmetricKey) throws {
        let input = try FileHandle(forReadingFrom: src)
        defer { try? input.close() }
