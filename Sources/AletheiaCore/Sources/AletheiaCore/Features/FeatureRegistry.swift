...
public struct FeatureRegistry {
    ...
    public let modules: [FeatureModule]
    ...
    public static func compose(tier: BuildTier, from all: [FeatureModule]) -> FeatureRegistry {
        var seen = Set<String>()
        let included = all
    ...
    public func contains(id: String) -> Bool { modules.contains { $0.id == id } }

    /// The module with `id`, if present.
    public func module(id: String) -> FeatureModule? { modules.first { $0.id == id } }

    /// The enabled module ids, in registry order.
    public var ids: [String] { modules.map(\.id) }
}
...
extension FeatureRegistry {
    ...
    public static let allModules: [FeatureModule] = [
