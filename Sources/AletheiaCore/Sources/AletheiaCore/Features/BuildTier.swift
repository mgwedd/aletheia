...
public enum BuildTier: String, CaseIterable, Comparable, Codable {
    ...
    public static func < (lhs: BuildTier, rhs: BuildTier) -> Bool { lhs.rank < rhs.rank }

    /// Whether a build at this tier ships a module declared at `moduleTier`.
    /// A build includes everything at or below its own tier.
    public func includes(_ moduleTier: BuildTier) -> Bool { moduleTier <= self }
    ...
    /// back to `.mvp`.
    public static var current: BuildTier {
